...
# Load data
f_f_mkt_smb <- read_csv('F-F-Annual_SMB.csv')
...
combined_data <- cbind(aligned_excess_returns, SMB = aligned_mkt_smb$SMB)

# Check the structure of the combined data
str(combined_data)

# extract portfolio data
portfolio_excess_returns <- combined_data %>% select(-YEAR, -SMB)
...
for(portfolio in colnames(portfolio_excess_returns)) {
  ...
  formula <- as.formula(paste("`", portfolio, "`", "~ SMB", sep = ""))
  
  # Perform the linear regression
  model <- lm(formula, data = combined_data)
  
  # Extract the beta coefficient for SMB
  beta <- coef(model)["SMB"]
...
betas_smb <- data.frame(Portfolio = names(betas), SMB_Beta = unlist(betas))
