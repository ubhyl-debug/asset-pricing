...
# Load data
f_f_mkt_rf <- read_csv('F-F-Annual_Mkt_Rf.csv')
...
combined_data <- cbind(aligned_excess_returns, Mkt_RF = aligned_mkt_rf$Mkt_RF)

# Check the structure of the combined data
str(combined_data)

# extract portfolio data
portfolio_excess_returns <- combined_data %>% select(-YEAR, -Mkt_RF)
...
for(portfolio in colnames(portfolio_excess_returns)) {
  ...
  formula <- as.formula(paste("`", portfolio, "`", "~ Mkt_RF", sep = ""))
  
  # Perform the linear regression
  model <- lm(formula, data = combined_data)
  
  # Extract the beta coefficient for Mkt_RF
  beta <- coef(model)["Mkt_RF"]
...
betas_mktrf <- data.frame(Portfolio = names(betas), Mkt_RF_Beta = unlist(betas))
